...
if(
    ...
    $from = $db->Real(($_POST['fromex']));
    $to = $db->Real(($_POST['toox']));
    $description = $db->Real(($_POST['descriptionx']));
    session_start();
    if (isset($_SESSION["login"]) && $_SESSION["login"] == true) {
        $freelancerid = $_SESSION["submitid"];
        $id = $db->Gid();
        $insertex = mysqli_query($db->connect(), "INSERT INTO workex(id, position, company, fromex, toox, description, freelancer_id, status)
 VALUES ('$id','$position','$company','$from','$to','$description','$freelancerid',0)");
